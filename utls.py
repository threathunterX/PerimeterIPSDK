...
CURVER = ""
__all__ = ["DownLoad", "InitialPackage", "update"]
...
class InitialPackage(object):
            ...
            with open("./task/{}".format(filename), "r") as f:
                for lines in cut_file(f, 100000):
                    ...
                    if database == "mongodb":
                        ...
                        n = 0
                        while n < 5:
                            if IpToMongoDB.get_instance().batch_insert(json_data_list):
    ...
    def process(self, database):
        ...
        curver, filename = self.get_initial_resp()
...
class IpToMysql(object):
    ...
    def create_table(self):
        ...
        sql = "create table if not exists `{table}` " \
              ...
              "`risk_tag` varchar(2000) COMMENT '风险标签'," \
        ...
        try:
            ...
            with conn.cursor() as cursor:
                for data in param_list:
                    ...
                    if rows != 0:
                        delete_sql = "DELETE  from `{table}` where ip=%(ip)s"
                        delete_sql = delete_sql.format(table=self.table)
...
def update(database, curver):
    def get_download_link():
        ...
        result = download_obj.get_upgrade_resp({"curver": curver, "type": 0})
        link = result.get("link", "")
        if link:
            n = 0
            while n < 10:
                if download_obj.download(url=link, file_name=filename):
